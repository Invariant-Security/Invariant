...
def _assert_all_pass_except(findings, failing_external_id):
    """One check per CHECKS entry runs per target -- rather than hardcode
    every external_id (they drift per document, e.g. Debian 13 uses 5.1.21
    where the rest use 5.1.20) or a check count that grows as more checks
    are added, just assert the demo's "one problem per machine" story:
    exactly one FAIL, everything else PASS.
    """
